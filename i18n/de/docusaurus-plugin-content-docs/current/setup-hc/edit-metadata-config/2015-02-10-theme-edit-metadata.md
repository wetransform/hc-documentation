...
Diese Option ermöglicht die Verwendung des Wertes `information` im Element `transferOptions` in den Metadaten und deren Verweis auf das WFS-Capabilities-Dokument. Der Wert "information" ist Teil der Codeliste `CI_OnLineFunctionCode`.
Im Abschnitt 4.3.1 der ["Konventionen zu Metadaten" der GDI-DE](https://www.gdi-de.org/download/AK_Metadaten_Konventionen_zu_Metadaten.pdf) (v2.2.1) wird die Verwendung des Wertes `information` empfohlen. Nutzende, die die Einhaltung der GDI-DE-Metadatenkonventionen mit Hilfe der GDI-DE-Testsuite prüfen, erhalten eine Warnung, falls ein anderer Wert verwendet wird. Im Technischen Leitfaden für die Implementierung von Metadaten für INSPIRE-Datensätze und -Dienste auf der Grundlage von ISO/TS 19139:2007, Version 2.2.0, TG Recommendation 3.5: metadata/2.0/rec/sds/resource-type-url-target wird ein Beispiel angeführt, in dem die Metadaten und deren Verweis auf das WFS-Capabilities-Dokument den Wert "download" verwenden. Der Standardwert von hale»connect ist daher "download", der Standardwert kann jedoch durch die Verwendung dieses Profils überschrieben werden.
